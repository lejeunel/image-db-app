...
from ... import db
from ...models import (Cell, Compound, Item, ItemTagAssociation, Modality,
                       Plate, Section, Stack, StackModalityAssociation, Tag,
                       TimePoint)
...
def get_items_with_meta():
    ...
    # aggregate tags functions (sqlite and postgre)
    ...
    items = (
        db.session.query(
            ...
            TimePoint.time.label("tp_time"),
            TimePoint.id.label("timepoint_id"),
            Section.id.label("section_id"),
            Plate.id.label("plate_id"),
            func.string_agg(Tag.name, literal_column("','"))
        )
        ...
        .filter(
            ...
            TimePoint.id.label("timepoint_id"),
            Section.id.label("section_id"),
            # func.string_agg(Tag.name, literal_column("','")).label("tags"),
        )
